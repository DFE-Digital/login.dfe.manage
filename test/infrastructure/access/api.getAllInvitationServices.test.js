...
describe("when calling the getAllInvitationServices function", () => {
  beforeEach(() => {
    fetchApi.mockReset();
  ...
  it("then it should call associated-with-user resource with user id", async () => {
    await getAllInvitationServices(invitationId, correlationId);

    expect(fetchApi.mock.calls).toHaveLength(1);
    expect(fetchApi.mock.calls[0][0]).toBe(
      "http://access.test/invitations/inv-1/services",
    );
  ...
  it("should return undefined on a 404 response", async () => {
    ...
    const result = await getAllInvitationServices(invitationId, correlationId);
