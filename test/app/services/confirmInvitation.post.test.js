...
const postConfirmInvitation =
  require("./../../../src/app/services/confirmInvitation").post;
...
describe("when inviting a new user", () => {
  ...
  it("then it should create an invite if no uid exists in params", async () => {
    ...
    expect(createInvite.mock.calls[0][4]).toBe(
      "https://services.unit.test/auth",
    );
  });
  ...
  it("then it should should audit an invited user if no uid in params", async () => {
    ...
    expect(logger.audit.mock.calls[0][0]).toBe(
      "user@unit.test (id: user1) invited john.doe@unit.test to organisation name (id: org1) (id: inv-invite1)",
    );
  ...
  it("then a flash message is displayed for a user being invited", async () => {
    ...
    expect(res.flash.mock.calls[0][1]).toBe(
      `Invitation email sent to john.doe@unit.test`,
    );
  });
  ...
  it("then a flash message is displayed for existing user service added", async () => {
    ...
    expect(res.flash.mock.calls[0][0]).toBe("info");
    expect(res.flash.mock.calls[0][1]).toBe(
      `service name added to John Doe at organisation name`,
    );
  });
