jest.mock("./../../../../src/infrastructure/config", () =>
  require("../../../utils").configMockFactory(),
);
jest.mock("login.dfe.api-client/services");

const { getRequestMock, getResponseMock } = require("../../../utils");
const getConfirmCreateNewPolicy = require("../../../../src/app/services/createNewPolicy/getConfirmCreateNewPolicy");
const { getServiceRaw } = require("login.dfe.api-client/services");
const res = getResponseMock();

// Fields removed for brevity.
const service = {
  id: "service-1",
  name: "Service One",
};

describe("when calling the getCreateNewPolicyCondition function", () => {
  let req;

  beforeEach(() => {
    req = getRequestMock({
      params: {
        sid: "service-1",
      },
      session: {
        createNewPolicy: {
          name: "Test Policy",
          role: {
            roleName: "Test role",
            roleCode: "Test code",
          },
          condition: {
            condition: "organisation.urn",
            operator: "is",
            value: "123456",
          },
        },
      },
    });

    getServiceRaw.mockReset();
    getServiceRaw.mockResolvedValue(service);

    res.mockResetAll();
  });

  it("should return the createNewPolicyCondition view", async () => {
    await getConfirmCreateNewPolicy(req, res);

    expect(res.render.mock.calls.length).toBe(1);
    expect(res.render.mock.calls[0][0]).toBe(
      "services/views/confirmCreateNewPolicy",
    );
  });

  it("should include the following in the model on success", async () => {
    await getConfirmCreateNewPolicy(req, res);

    expect(res.render.mock.calls[0][1]).toStrictEqual({
      backLink: `/services/${req.params.sid}/policies/create-new-policy-condition`,
      cancelLink: `/services/${req.params.sid}/policies`,
      serviceId: "service-1",
      csrfToken: "token",
      currentNavigation: "policies",
      model: {
        name: "Test Policy",
        role: {
          roleCode: "Test code",
          roleName: "Test role",
        },
        condition: {
          condition: "organisation.urn",
          operator: "is",
          value: "123456",
        },
      },
      service,
      userRoles: [],
    });
  });

  it("should call getServiceRaw with the correct parameters", async () => {
    await getConfirmCreateNewPolicy(req, res);

    expect(getServiceRaw).toHaveBeenCalledWith({
      by: {
        serviceId: "service-1",
      },
    });
  });

  it("should redirect to policies if there is no data in the session", async () => {
    req = getRequestMock({
      params: {
        sid: "service-1",
      },
    });
    await getConfirmCreateNewPolicy(req, res);

    expect(res.redirect.mock.calls[0][0]).toBe("/services/service-1/policies");
  });
});
