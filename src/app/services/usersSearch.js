...
const { userStatusMap, lastLoginIntervalsMap } = require('../../infrastructure/utils/index');
const {
  getUserServiceRoles,
  unpackMultiSelect,
  buildFilters,
...
const search = async (req) => {
  ...
  let filters = buildFilters(paramsSource, 'organisationCategories', 'lastLogin', 'statusId');
  ...
  if (Object.prototype.hasOwnProperty.call(filters, 'lastLogin') && filters.lastLogin.length > 0) {
    const mappedLoginIntervalsToDates = mapLastLoginValuesToDateValues(filters.lastLogin);
    mappedToDateFilters = { ...mappedToDateFilters, lastLogin: mappedLoginIntervalsToDates };
  }
...
const getFiltersModel = async (req) => {
  ...
  if (showFilters) {
    ...
    const selectedLastLoginIntervals = unpackMultiSelect(paramsSource.lastLogin);
    lastLoginIntervals = lastLoginIntervalsMap.map((interval) => ({
      id: interval.id,
      name: interval.name,
      isSelected: selectedLastLoginIntervals.find((x) => x === interval.id.toString()) !== undefined,
    }));
